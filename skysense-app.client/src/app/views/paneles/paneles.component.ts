...
export class PanelesComponent {
  _panelesPaginados:Paginacion<PanelSimple> = new Paginacion<PanelSimple>();
  _paginasArray:number[] = [];

  #UserService:UserService = inject(UserService);
  #PanelesService:InstalacionesService = inject(InstalacionesService);
  #tostadaService = inject(ToastService);

  _error:boolean = false;

_editandoRenglon: number = -1;
_panelEditando!:PanelModel;
...
_ordenandoPorNombreAscendente:boolean|undefined;
  ...
  ObtenerPaneles(pagina: number = 1){

    this.#PanelesService.mObtenerPanelesPaginados(pagina,10,this.buscador?.toUpperCase()).subscribe({
      next: (valor)=>{
        if(valor.exito){
          this._panelesPaginados = valor.data;
          this._paginasArray = Array.from({length: valor.data.numeroPaginas}, (_, i) => i + 1);
          //this._haciendoPeticion = false;
        }else{
          this.#tostadaService.GeneraAlertaToastDirecta("Error al obtener paneles", valor.mensaje, NivelAlerta.Advertencia);
        }
      },
      error: (error)=>{
        this.#tostadaService.GeneraAlertaToastDirecta("Error al obtener paneles", "Ocurrió un error inesperado al procesar la solicitud.", NivelAlerta.Peligro);
      },
      complete: ()=>{
        this._haciendoPeticion = false;
      }
  ...
  //   this._paneles.sort((a,b)=> (this._ordenandoPorNombreAscendente?1:-1)*a.marca.localeCompare(b.marca));
  // }
